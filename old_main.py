...
class airfoil:
    ...
    def read_in(self):
        ...
        self.do_offset_in_z             = json_vals["appellian"]["do_offset_in_z_plane"]
    ...
    def select_VPM_solution_mapping(self, D:float):
        ...
        for i, theta_stag_chi_rad in enumerate(tqdm(theta_stag_list, desc="Calculating appellians for Polyfit")):
            ...
            if self.do_vpm_analytic_partials or self.do_vpm_numerical_partials:
                ...
                print("vpm gamma   = ", vpms[i].gamma_total)
                print("    gamma   = ", gamma_list[i])
                print("   vpm CL   = ", vpms[i].CL)
                print("       CL   = ", 2.*np.pi*( (np.sin(self.alpha_rad) + self.zeta_0.imag*np.cos(self.alpha_rad)/np.sqrt(self.radius**2 - self.zeta_0.imag**2) ) /
                             (1. + self.zeta_0.real / (np.sqrt(self.radius**2 - self.zeta_0.imag**2) - self.zeta_0.real) )))
            ...
            if self.do_offset_in_z:
                appellian_offset_in_z = mappings[i].calc_appellian_offset_in_z(gamma_list[i], self.type_of_integration)
                appellian_list_offset_in_z.append(appellian_offset_in_z)
        ...
        if self.do_offset_in_z:
            print("appellian list offset_in_z = ", [float(x) for x in appellian_list_offset_in_z])
        
            self.coeffs_offset_in_z = np.polyfit(gamma_list, appellian_list_offset_in_z, 4)
            # print("Coefficients = ", self.coeffs_line)
                
            self.d_coeffs_offset_in_z = np.polyder(self.coeffs_offset_in_z)

            # find zeros of derivative polynomial
            roots_gammas_offset_in_z = np.real(np.roots(self.d_coeffs_offset_in_z))
            print("roots gamma offset_in_z = ", roots_gammas_offset_in_z)
            roots_appellians_offset_in_z = np.zeros(len(roots_gammas_offset_in_z))

            # check which min candidate gives the lowest appellian
            root_mappings = []
            for i, gamma in enumerate(tqdm(roots_gammas_offset_in_z, desc="Calculating appellians offset in z for root selection")):
                if abs(gamma/(4.*np.pi*self.V_inf*self.radius)) > 1.0:
                    print("Error, a gamma root was too large. Setting gamma/(4.*np.pi*self.V_inf*self.radius == 1")
                    gamma = (4.*np.pi*self.V_inf*self.radius)
                theta_stag_chi_rad = self.alpha_rad - np.arcsin(gamma/(4.*np.pi*self.V_inf*self.radius))
                root_mappings.append(jouk(self.shape_D, self.zeta_0, self.alpha_rad, self.V_inf, self.radius, self.num_panels, theta_stag_chi_rad, self.clustering, self.ofs, self.offset_coef))
                roots_appellians_offset_in_z[i] = root_mappings[i].calc_appellian_offset_in_z(gamma, self.type_of_integration)
            

            # get index of min value
            print("roots appellians offset_in_z = ", roots_appellians_offset_in_z)
            self.min_appellian_offset_in_z = roots_appellians_offset_in_z[np.argmin(roots_appellians_offset_in_z)]
            self.gamma_polyfit_offset_in_z = roots_gammas_offset_in_z[np.argmin(roots_appellians_offset_in_z)] 
            self.theta_selected = -np.arcsin(self.gamma_polyfit_offset_in_z/(4.*np.pi*self.V_inf*self.radius)) + self.alpha_rad
            self.mapping = jouk(self.shape_D, self.zeta_0, self.alpha_rad, self.V_inf, self.radius, self.num_panels, self.theta_selected, self.clustering, self.ofs, self.offset_coef, False, True)
            appellian = self.mapping.calc_appellian_offset_in_z(self.gamma_polyfit_offset_in_z, self.type_of_integration)
            print("theta stag selected = ", self.theta_selected)

            print("\nIntegration = ", self.type_of_integration, ",    num_panels = ", self.num_panels, "clustering = ", self.clustering)
            print(" min appellian offset_in_z analytic = ", self.min_appellian_offset_in_z)
            print("gamma selected offset_in_z analytic = ", self.gamma_polyfit_offset_in_z)
            ...
            self.gamma_kutta = self.mapping.gamma_Kutta
            self.min_appellian = self.min_appellian_offset_in_z
            self.gamma_selected = self.gamma_polyfit_offset_in_z
        ...
        if self.do_vpm_numerical_partials:
            appellian_list_vpm_numerical = [float(item) for item in appellian_list_vpm_numerical] 
