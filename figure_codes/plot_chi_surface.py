...
def plot(x, y, theta_stag_deg, gamma, figs_dir, make_svg, make_pdf, show):

    apply_plot_settings()
    fig, ax = plt.subplots(**default_subplot_settings)

    ax.plot(x,y, color='0.0', linestyle="-")  # Dash style 1
    ax.scatter(x,y, color='0.0', s=1)  # Dash style 1
    ax.scatter(0.0, 0.0, marker="+", color='0.0', s=20)  # Dash style 1

    ax.set_title(r"$\chi$  plane")
    ...
    # test arc drawing settings
    # theta_stag_deg = -19
    
    if abs(theta_stag_deg) > 25:
        label_radius = 0.7
        arc_pos = 0.9
    else:
        label_radius = 1.3
    ...
    if theta_stag_deg >= 0.0:
        ...
        if abs(theta_stag_deg) < 25:
            label_angle = theta_stag_deg*.33
        else:
            label_angle = theta_stag_deg*.5
    elif theta_stag_deg < 0.0:
        arc = Arc((0.0, 0.0),arc_pos, arc_pos,  angle=0, theta1=theta_stag_deg, theta2=0.0, color="k")
        if abs(theta_stag_deg) < 25:
            label_angle = theta_stag_deg*.66
        else:
            label_angle = theta_stag_deg*.6
    ax.add_patch(arc)
    ...
    ax.scatter(np.cos(np.radians(theta_stag_deg)), np.sin(np.radians(theta_stag_deg)), color='r',s=3, zorder=10)
    ...
    plt.savefig(f"{figs_dir}/chi_test_gamma=" + str(gamma) + ".png", format='png')

    if make_svg:
        plt.savefig(f"{figs_dir}/chi_test_gamma=" + str(gamma) + ".svg", format='svg')
    if make_pdf:
        plt.savefig(f"{figs_dir}/chi_test_gamma=" + str(gamma) + ".pdf", format='pdf')
