...
# def plot(x, y, gamma, norm_x, norm_y, figs_dir, make_svg, make_pdf, show):
def plot(surface, contour, gamma, figs_dir, make_svg, make_pdf, show):
    ...
    # ax.scatter(x[0], y[0], color='r',s=3, zorder=10)
    # ax.scatter(0.0, 0.0, marker="+", color='0.0', s=20)  # Dash style 1

    ax.plot(contour.real,contour.imag, color='0.0', linestyle="-")  # Dash style 1
    # ax.scatter(contour.real,contour.imag, color='0.0', s=0.5)  # Dash style 1
    ...
    # plt.quiver(x, y, norm_x, norm_y, angles ='uv', color = "r", width = 0.003, headwidth=3, headlength=6, headaxislength=5)


    ax.set_title(r"$z$  plane")
    ...
    plt.savefig(f"{figs_dir}/z_test_gamma={gamma:.5e}.png", format='png')

    if make_svg:
        plt.savefig(f"{figs_dir}/z_test_gamma={gamma:.5e}.svg", format='svg')
    if make_pdf:
        plt.savefig(f"{figs_dir}/z_test_gamma={gamma:.5e}.pdf", format='pdf')
